...
const W3 = "rgba(240,234,224,0.6)";

const slides: Slide[] = [
  { id: 0, type: "title", accent: "#C8A882", title: "Предложение по концепциям\nДня рождения для Галины" },
  {
  ...
  {
    id: 3, type: "concept", accent: "#9B7EC8", num: "02",
  ...
  {
    id: 4, type: "conceptB", accent: "#9B7EC8", num: "02",
    tag: "Вариант 2", title: "Архетипы женщины",
  ...
  {
    id: 5, type: "concept3", accent: "#C8A882", num: "03",
  ...
  {
    id: 6, type: "concept3B", accent: "#C8A882", num: "03",
    tag: "Вариант 3", title: "Машина времени",
  ...
  {
    id: 7, type: "concept4", accent: "#C8A96E", num: "04",
  ...
  {
    id: 8, type: "concept4B", accent: "#C8A96E", num: "04",
    tag: "Вариант 4", title: "Зов Алтая",
  ...
  {
    id: 9, type: "concept5", accent: "#D4A85A", num: "05",
  ...
  {
    id: 10, type: "concept5B", accent: "#D4A85A", num: "05",
    tag: "Вариант 5", title: "Режиссёрская версия",
  ...
  {
    id: 11, type: "final", accent: "#C8A882",
...
export default function Index() {
  ...
  const Tag = ({ label, accent }: { label: string; accent: string }) => (
    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <span style={{ fontFamily: F, fontWeight: 500, fontSize: "clamp(8px, 0.85vw, 11px)", letterSpacing: "0.28em", textTransform: "uppercase", color: accent, opacity: 0.85 }}>{label}</span>
    </div>
  );
    ...
    <div className="w-screen h-screen overflow-hidden flex items-center justify-center" style={{ background: "#080807" }}>
      ...
      >
        ...
        >
          ...
          {/* ── TITLE — центр с горизонтальной линией ── */}
          {slide.type === "title" && (() => {
                ...
                <div style={{ flex: "0 0 55%", display: "flex", flexDirection: "column", justifyContent: "center", padding: "5% 5% 5% 7%", gap: "clamp(10px, 1.5vw, 18px)", borderRight: "1px solid rgba(255,255,255,0.05)" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "clamp(6px, 0.9vw, 11px)" }}>
                    {s.paragraphs.map((p, i) => (
                      <p key={i} style={{ fontFamily: F, fontWeight: 400, fontSize: "clamp(10px, 1.15vw, 14px)", lineHeight: 1.7, color: W3, margin: 0 }}>{p}</p>
                    ))}
                  </div>
                  <div style={{ marginTop: "auto", paddingTop: "clamp(8px, 1vw, 12px)", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                    <span style={{ fontFamily: F, fontWeight: 400, fontSize: "clamp(9px, 1vw, 12px)", color: s.accent, opacity: 0.65 }}>↳ {s.footer}</span>
                  </div>
                </div>
                <div style={{ flex: 1, position: "relative", overflow: "hidden" }}>
                  <img src={s.image} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", filter: "brightness(0.7) saturate(0.85)" }} />
                  <div style={{ position: "absolute", inset: 0, background: "linear-gradient(135deg, rgba(15,14,13,0.55) 0%, transparent 60%)" }} />
          ...
          {/* ── CONCEPT B — full-bleed, текст поверх слева ── */}
          {slide.type === "conceptB" && (() => {
          ...
          {/* ── CONCEPT 3 — картинка слева, текст справа ── */}
          {slide.type === "concept3" && (() => {
            ...
            return (
              <div style={{ width: "100%", height: "100%", display: "flex" }}>
                ...
                <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "5% 7% 5% 5%", gap: "clamp(10px, 1.4vw, 17px)" }}>
                  ...
                  <div style={{ width: 32, height: 1, background: `${s.accent}80` }} />
                  <p style={{ fontFamily: F, fontWeight: 400, fontSize: "clamp(10px, 1.1vw, 13px)", lineHeight: 1.7, color: W3, margin: 0 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "clamp(5px, 0.7vw, 9px)", marginTop: 4 }}>
                    {s.rooms.map((r, i) => (
                      <div key={i} style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
                        ...
                        <span style={{ fontFamily: F, fontWeight: 400, fontSize: "clamp(10px, 1.15vw, 14px)", color: "rgba(240,234,224,0.75)" }}>{r}</span>
                      </div>
                    ))}
                  </div>
                  <div style={{ marginTop: "auto", paddingTop: "clamp(8px, 1vw, 12px)", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                    <span style={{ fontFamily: F, fontWeight: 400, fontSize: "clamp(9px, 1vw, 12px)", color: s.accent, opacity: 0.6 }}>↳ {s.footer}</span>
                  </div>
                </div>
              </div>
            );
          ...
          {/* ── CONCEPT 3B — full-bleed, текст справа ── */}
                ...
                <div style={{ position: "relative", height: "100%", display: "flex", flexDirection: "column", justifyContent: "flex-end", padding: "0 7% 6% 7%" }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "clamp(20px, 3vw, 48px)", alignItems: "end" }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "clamp(10px, 1.4vw, 17px)", paddingBottom: 4 }}>
                      <p style={{ fontFamily: F, fontWeight: 500, fontSize: "clamp(12px, 1.45vw, 18px)", color: s.accent, lineHeight: 1.4, margin: 0 }}>{s.lead}</p>
                      <div style={{ width: 32, height: 1, background: `${s.accent}80` }} />
                      <p style={{ fontFamily: F, fontWeight: 400, fontSize: "clamp(10px, 1.15vw, 14px)", lineHeight: 1.7, color: W3, margin: 0 }}>{s.body}</p>
                      <span style={{ fontFamily: F, fontWeight: 400, fontSize: "clamp(9px, 1vw, 12px)", color: s.accent, opacity: 0.6 }}>↳ {s.footer}</span>
                    </div>
                  </div>
                </div>
          ...
          {/* ── CONCEPT 4B — full-bleed, текст слева ── */}
            ...
            return (
              <div style={{ width: "100%", height: "100%", display: "flex" }}>
                <div style={{ flex: "0 0 55%", display: "flex", flexDirection: "column", justifyContent: "center", padding: "5% 5% 5% 7%", gap: "clamp(10px, 1.4vw, 17px)", borderRight: "1px solid rgba(255,255,255,0.05)" }}>
                  ...
                  <div style={{ width: 32, height: 1, background: `${s.accent}80` }} />
                  <p style={{ fontFamily: F, fontWeight: 400, fontSize: "clamp(10px, 1.15vw, 14px)", lineHeight: 1.75, color: W3, margin: 0 }}>{s.para1}</p>
                  <p style={{ fontFamily: F, fontWeight: 400, fontSize: "clamp(10px, 1.15vw, 14px)", lineHeight: 1.75, color: W3, margin: 0 }}>{s.para2}</p>
                  <div style={{ marginTop: "auto", paddingTop: "clamp(8px, 1vw, 12px)", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                    <span style={{ fontFamily: F, fontWeight: 600, fontSize: "clamp(8px, 0.85vw, 11px)", letterSpacing: "0.3em", textTransform: "uppercase", color: s.accent, opacity: 0.7 }}>Мотор!</span>
                  </div>
                </div>
                <div style={{ flex: 1, position: "relative", overflow: "hidden" }}>
                  <img src={s.image} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center 30%", filter: "brightness(0.6) saturate(0.8)" }} />
          ...
          {/* ── CONCEPT 5B — full-bleed, текст слева ── */}
            ...
            return (
              <div style={{ width: "100%", height: "100%", position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ position: "relative", height: "100%", display: "flex", flexDirection: "column", justifyContent: "center", padding: "6% 52% 6% 7%", gap: "clamp(14px, 2vw, 26px)" }}>
                  ...
                  <h2 style={{ fontFamily: F, fontWeight: 600, fontSize: "clamp(28px, 4.6vw, 58px)", lineHeight: 1.0, color: W1, margin: 0, letterSpacing: "-0.03em" }}>{s.title}</h2>
                  <div style={{ width: 32, height: 1, background: `${s.accent}80` }} />
                  <p style={{ fontFamily: F, fontWeight: 400, fontSize: "clamp(11px, 1.3vw, 16px)", lineHeight: 1.8, color: W2, margin: 0 }}>{s.body}</p>
                </div>
              </div>
